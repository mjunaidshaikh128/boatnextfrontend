...
const Chat = () => {
  ...
  const getUsername = async (userId: number) => {
    const res = await fetch(`http://localhost:3000/user/${userId}`, { cache: "no-cache" });
    if (!res.ok) {
  ...
  const join = async () => {
    const response = await socket.emit("join", { name }, (resp: any) => {
      setJoined(true);
    });
  };
  let timeout;
  ...
  useEffect(() => {
    ...
    if (messages.length < 1) {
      socket.emit("findAllMessages", { senderId: localStorage.getItem('userId'), receiverId: "4" }, (response: any) => {
        // console.log("Response", response)
        // console.log("Sender", localStorage.getItem('userId'));
  ...
  return (
    <>
      ...
      {isOpened && (
        <div
          style={divStyle2}
          className="fixed bottom-[calc(4rem+1.5rem)] right-0 mr-4 bg-white p-6 rounded-lg border border-[#e5e7eb] w-[440px] h-[634px] overflow-y-scroll"
        >
          {/* <!-- Heading --> */}
          ...
          {/* <!-- Chat Container --> */}
          <div className="pr-4 h-[474px]" style={divStyle}>
            {/* <!-- Chat Message AI --> */}
